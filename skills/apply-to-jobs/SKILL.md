...
3. **Fit gate.** Skip on a hard conflict: work authorization or citizenship the profile cannot meet, degree or graduation window, earliest start date, mandatory experience the resume clearly lacks, an excluded role family, or any user-required location, remote-work, compensation, or employment-type constraint. Log it: `jobs.py add ... --status skipped --reason "<one line>"`. Soft preferences and nice-to-have skills are not automatic skips. If a required constraint cannot be established from the posting, record `needs_input` rather than assuming it is satisfied.
